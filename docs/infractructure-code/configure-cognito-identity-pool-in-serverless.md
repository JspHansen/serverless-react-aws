...
Now it looks like there is a whole lot going on here. But it is pretty much exactly what we did back in the [Create a Cognito identity pool](../deploying-backend/create-a-cognito-identity-pool.md) chapter. It’s just that CloudFormation can be a bit verbose and can end up looking a bit intimidating.
